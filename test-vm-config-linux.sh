#!/bin/bash
#
# Ghost AI System - VM Test Configuration (LINUX)
#
# ⚠️  PLATFORM: This script is for LINUX systems only
# For macOS: use test-vm-config-macos.sh
# For Windows: use test-vm-config-windows.ps1
#
# This script creates and manages a QEMU/KVM virtual machine for testing
# the Ghost AI installation process without affecting real hardware.
#
# Requirements:
# - Linux host (Ubuntu, Debian, Fedora, Arch, etc.)
# - QEMU/KVM installed (apt install qemu-system-x86 qemu-utils qemu-kvm)
# - ~300GB free disk space
# - 16GB+ RAM recommended (8GB minimum)
# - CPU with virtualization support (Intel VT-x or AMD-V)
#
# Usage:
#   ./test-vm-config-linux.sh create    - Create a new test VM disk
#   ./test-vm-config-linux.sh boot-iso  - Boot from Ubuntu ISO for installation
#   ./test-vm-config-linux.sh boot      - Boot the installed system
#   ./test-vm-config-linux.sh snapshot  - Create a snapshot of current state
#   ./test-vm-config-linux.sh restore   - Restore from latest snapshot
#   ./test-vm-config-linux.sh clean     - Remove all VM files
#
...
create_vm() {
    ...
    log "Next: Run './test-vm-config-linux.sh boot-iso' to install Ubuntu"
}
...
boot_iso() {
    ...
    if [ ! -f "$VM_DISK" ]; then
        log_error "VM disk not found. Run './test-vm-config-linux.sh create' first"
        exit 1
    fi

    if [ ! -f "$UBUNTU_ISO" ]; then
        log_error "Ubuntu ISO not found at: $UBUNTU_ISO"
        log "Download it or run './test-vm-config-linux.sh create'"
        exit 1
    fi
    ...
    log "4. After installation, shutdown and run './test-vm-config-linux.sh boot'"
    log "================================="
    log ""
...
# Show usage
usage() {
    echo "Ghost AI System - VM Test Configuration (LINUX)"
    echo ""
    echo "⚠️  PLATFORM: Linux only (use test-vm-config-macos.sh for macOS)"
    echo ""
