...
echo ""
echo "Ghost AI Repository Checks:"
GHOST_AI_DIR="${GHOST_AI_DIR:-$HOME/Ghost-AI}"
check "Ghost AI directory exists" "[ -d $GHOST_AI_DIR ]"
check "vm-quick-install.sh exists" "[ -f $GHOST_AI_DIR/vm-quick-install.sh ]"
check "vm-quick-install.sh is executable" "[ -x $GHOST_AI_DIR/vm-quick-install.sh ]"
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo "  cd $GHOST_AI_DIR && ./vm-quick-install.sh"
else
