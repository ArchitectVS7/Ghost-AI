#!/bin/bash
#
# Ghost AI System - VM Test Configuration (macOS)
#
# ⚠️  PLATFORM: This script is for macOS systems only
# For Linux: use test-vm-config-linux.sh
# For Windows: use test-vm-config-windows.ps1
#
...
#
# Usage:
#   ./test-vm-config-macos.sh create    - Create a new test VM disk
#   ./test-vm-config-macos.sh boot-iso  - Boot from Ubuntu ISO for installation
#   ./test-vm-config-macos.sh boot      - Boot the installed system
#   ./test-vm-config-macos.sh snapshot  - Create a snapshot of current state
#   ./test-vm-config-macos.sh restore   - Restore from latest snapshot
#   ./test-vm-config-macos.sh clean     - Remove all VM files
#
...
# Detect architecture and available accelerators
ARCH=$(uname -m)
QEMU_BINARY="qemu-system-x86_64"

# Check if HVF is available, otherwise use TCG
if qemu-system-x86_64 -accel help 2>&1 | grep -q "hvf"; then
    ACCEL="-accel hvf"
else
    ACCEL="-accel tcg"
    # Note: TCG is software emulation and will be slower
fi
...
check_deps() {
    ...
    command -v qemu-img &>/dev/null || missing+=("qemu")

    if [ ${#missing[@]} -gt 0 ]; then
        log_error "Missing dependencies: QEMU"
        log ""
        log "Install with Homebrew:"
        log "  brew install qemu"
        log ""
        log "Don't have Homebrew? Install it first:"
    ...
    if ! command -v brew &>/dev/null; then
        ...
        log_warning "You may need to add it: eval \"\$(/opt/homebrew/bin/brew shellenv)\""
    fi
}
...
create_vm() {
    ...
    if [ ! -f "$UBUNTU_ISO" ]; then
        ...
        log_warning "This is a ~6GB download and may take a while..."

        # Use curl instead of wget (more common on macOS)
        curl -L -o "$UBUNTU_ISO" "https://releases.ubuntu.com/24.04.1/ubuntu-24.04.1-desktop-amd64.iso"
    ...
    log ""
    log "Next: Run './test-vm-config-macos.sh boot-iso' to install Ubuntu"
}
...
boot_iso() {
    ...
    if [ ! -f "$VM_DISK" ]; then
        log_error "VM disk not found. Run './test-vm-config-macos.sh create' first"
        exit 1
    fi

    if [ ! -f "$UBUNTU_ISO" ]; then
        log_error "Ubuntu ISO not found at: $UBUNTU_ISO"
        log "Download it or run './test-vm-config-macos.sh create'"
        exit 1
    fi
    ...
    log "4. After installation, shutdown and run './test-vm-config-macos.sh boot'"
    log "================================="
    log ""
...
boot_headless() {
    ...
    QEMU_PID=$!
    echo $QEMU_PID > /tmp/ghost-ai-vm.pid

    log_success "VM started in background (PID: $QEMU_PID)"
    log "PID file: /tmp/ghost-ai-vm.pid"
    log "To stop: kill $QEMU_PID"
}
...
# Show usage
usage() {
    echo "Ghost AI System - VM Test Configuration (macOS)"
    echo ""
    echo "⚠️  PLATFORM: macOS only (use test-vm-config-linux.sh for Linux)"
    echo ""
